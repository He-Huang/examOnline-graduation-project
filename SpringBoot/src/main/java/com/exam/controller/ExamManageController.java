...
public class ExamManageController {
    ...
    public ApiResult findAll(@PathVariable("page") Integer page, @PathVariable("size") Integer size){
        ...
        Page<ExamManage> all = examManageService.findAll(pageable);
    ...
    public ApiResult findById(@PathVariable("examCode") Integer examCode){
        ...
        Optional<ExamManage> res = examManageService.findById(examCode);
    ...
    public ApiResult update(@RequestBody ExamManage exammanage){
        ExamManage res = examManageService.update(exammanage);
//        if (res == 0) {
...
//        }
    ...
    public ApiResult add(@RequestBody ExamManage exammanage){
        ExamManage res = examManageService.add(exammanage);
        if (res!=null) {
    ...
    //返回的是一个id值，表示最后一个试卷id，返回给前端达到自增效果
    @GetMapping("/examManagePaperId")
    public ApiResult findOnlyPaperId() {
        Integer res = examManageService.findOnlyPaperId();
        if (res != null) {
            return ApiResultHandler.buildApiResult(200,"请求成功",res);
        }
