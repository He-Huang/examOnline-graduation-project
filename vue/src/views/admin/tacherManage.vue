...
<template>
  <div class="all">
    ...
    ></el-pagination>
    <!-- 编辑对话框-->
    <el-dialog title="编辑教师信息" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
...
export default {
  ...
  methods: {
    getTeacherInfo() {
      //分页查询所有试卷信息
      this.$axios(
        `/api/teachers/${this.pagination.current-1}/${this.pagination.size}`
      )
        .then(res => {
    ...
    findByName(text){
      ...
      for(var key in content){
        if(content[key].teacherName.search(text) != -1){
